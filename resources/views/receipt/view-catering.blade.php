  ...
  <table class="table table-bordered">
	<tbody>
		...
		<tr>
			...
			<td>00440035</td>
			<td>{{$receipt[0]->catering_cost}}</td>
			<td>{{slash_decimal($cgst)}}</td>
			<td>{{slash_decimal(percent_amount($receipt[0]->catering_cost,$cgst))}}</td>
			<td>{{slash_decimal($sgst)}}</td>
			<td>{{slash_decimal(percent_amount($receipt[0]->catering_cost,$sgst))}}</td>
			<td>{{slash_decimal($receipt[0]->catering_cost + percent_amount($receipt[0]->catering_cost,$cgst+$sgst))}}</td>
		</tr>
		...
		<tr>
	        ...
	        <td>{{round($receipt[0]->catering_cost + $receipt[0]->security)}}</td>
		</tr>
			<tr>
	        <td style="border-top: none;border-right: none" colspan="8" class="right-align">CGST:</td>
	        <td style="border-top: none;">{{round(percent_amount($receipt[0]->catering_cost,$cgst))}}</td>
		</tr>
		<tr>
	        <td style="border-top: none;border-right: none" colspan="8" class="right-align">SGST:</td>
	        <td style="border-top: none;">{{round(percent_amount($receipt[0]->catering_cost,$sgst))}}</td>
		</tr>
			<tr>
	        ...
	        @if($receipt[0]->reverse_charges == '1')
	        	{{round(percent_amount($receipt[0]->catering_cost,$cgst+$sgst))}}
	        @else
	        	0 
	        @endif
	        </td>
		...
		<tr>
	        ...
	        <td style="border-top: none;">
	        {{round($receipt[0]->catering_cost + $receipt[0]->security + percent_amount($receipt[0]->catering_cost,$cgst+$sgst))}}
	        </td>
		</tr>
	</tbody>
  </table>
