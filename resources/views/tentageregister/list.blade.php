...
<div class="pageheader">
	<div class="media">
		...
		<h4>Tentage Register</h4>
		<ul class="breadcrumb">
		    <li><a href="{{ url('/') }}"><i class="glyphicon glyphicon-home"></i></a></li>
		    <li>Reports</li>
		    <li>Tentage Register</li>
		</ul>
		</div>
	</div>
</div>
...
<div id="page-wrapper">  
    ...
    <div class="row" style="margin-top: 10px">
		...
		<?php if($count>0 && $booking[0]->booking_id != ''){ ?>
		  ...
		  <h4 class="center-underline onlyprint">Tentage Register</h4>
		  	@if($_POST && $_POST>0)
			  ...
			  <table class="table table-bordered" style="border: 1px solid #ddd; margin-top: 15px">
			    ...
			    <tbody>
		          ...
		          <tr>
		            ...
		            <td><?php if($receipt[$key]->booking_id==$value->booking_id){?> {{$receipt[$key]->est_tentage}} <?php }?></td>
		            <td>{{ $value->rebate }}</td>
		            <td><?php if($receipt[$key]->booking_id==$value->booking_id){?> {{$receipt[$key]->st_tent}} <?php }?></td>
		            <td><?php if($receipt[$key]->booking_id==$value->booking_id){?> {{$receipt[$key]->vat_tent}} <?php }?></td>
		          </tr>
		          <?php $i++; } ?>
			    </tbody>
			  </table>
